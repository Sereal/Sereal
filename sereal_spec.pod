...
This is the Sereal specification version 3.00.
...
and when decoded as an unsigned 32 bit
integer on a little endian machine has a value of 0x6c72733d.

In protocol version 3, the magic string can be B<either> the v1/v2 "=srl"
B<or> it can be the same string with the high bit set on the "s": "=\xF3rl".
this makes it much easier to track down bugs caused by common encoding problems
in applications. In practice, this means that it
is very easy to detect that a Sereal v3 document has been erroneously
UTF-8 encoded because \xF3 is translated to two bytes \xC3\xB3. Decoders are
required to support both magic strings.
...
Up until now there have been versions 1, 2, and 3 of the Sereal protocol.
...
uses in many situations. The difference can be illustrated with the
following code:
...
As of protocol version 3 the SV_UNDEF tag is used to handle this special
case of undef so that Perl data structures can round trip properly.
Other languages are free to treat SV_UNDEF and UNDEF as is appropriate.
If there is an equivalent to PL_sv_undef then they should map SV_UNDEF
accordingly, otherwise they are free to treat SV_UNDEF the same as UNDEF.

=head1 PROTOCOL CHANGES

=head2 Protocol Version 3

In Sereal protocol version 3, there is a change to the Sereal magic string
that starts all Sereal documents: In order to make it much easier to track
down bugs caused by encoding problems in applications, the Sereal magic string
may now be either the pre-v3 "=srl" or instead also "=\xF3rl", so the same but
with the high bit set on the "s" character. In practice, this means that it
is very easy to detect that a Sereal v3 document has been erroneously
UTF-8 encoded because \xF3 is translated to two bytes \xC3\xB3. Decoders are
required to support both magic strings.
...
Additionally there is the new SV_UNDEF tag, used to represent Perl's
PL_sv_undef in certain edge cases. See L<Dealing with undefined values>
for details.
