...
are SHORT_BINARY and STR_BINARY, where binary means "raw bytes". The other
...
for strings of less than 32 characters long. Both STR_BINARY and STR_UTF8
use a varint to indicate the number of B<bytes> (octets) in the string.

=head4 Hash Keys

Hashs keys are always one of the string types, or a COPY tag referencing a
string.
