...
has a value of 0x6C72733D. Vesion 1 and 2 of the protocol require this
magic string.

In protocol version 3 the magic string has been changed to "=\xF3rl",
where \xF3 is "s" with the high bit set. The little endian integer form
of this string is 0x6C72F33D. Having a hight bit set in the magic string
makes it easy to detect when a Sereal document has been accidentally
UTF-8 encoded because the \xF3 is translated to \xC3\xB3.

Decoders are required to support the magic string associated to the
protocol versions they can decode, so if a decoder can handle v1, v2, and
v3 then it should handle both magic header.
...
=for autoupdater start
    ...
    CANONICAL_UNDEF   | "9"  |  57 | 0x39 | 0b00111001 | canonical, shared undef (PL_sv_undef in Perl, see notes)
    FALSE             | ":"  |  58 | 0x3a | 0b00111010 | false (PL_sv_no in Perl)
    TRUE              | ";"  |  59 | 0x3b | 0b00111011 | true  (PL_sv_yes in Perl)
    MANY              | "<"  |  60 | 0x3c | 0b00111100 | <LEN-VARINT> <TYPE-BYTE> <TAG-DATA> - repeated tag (not done yet, perhaps will be implemented in version 4)
